...
int usrmotWriteEmcmotCommand(emcmot_command_t * c)
{
	...
	char msg[124];
	sprintf(msg, "the  motion type is: %d ,and the motion id :%d\n", c->motion_type, c->id);
	fputs(msg, writeMsg);
	fflush(writeMsg);
	fclose(writeMsg);
	emcmot_status_t s;
...
{
	FILE *writeMsg = fopen("printMsg.txt", "a+");
	fputs("printEmcPose(EmcPose * pose) \n", writeMsg);
	char msg[512];
	sprintf(msg, "x=%f\ty=%f\tz=%f\tu=%f\tv=%f\tw=%f\ta=%f\tb=%f\tc=%f",
...
int usrmotInit(const char *modname)
{
	...
	retval = rtapi_shmem_getptr(shmem_id, (void **)&emcmotStruct, 0);
	if (retval < 0) {
