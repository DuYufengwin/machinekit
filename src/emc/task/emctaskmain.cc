...
static int emcTaskIssueCommand(NMLmsg * cmd)
{
	...
	fputs("enter the emcTaskIssueCommand() \n", writeMsg);
	fflush(writeMsg);
	fclose(writeMsg);
	int retval = 0;
	int execRetval = 0;
...
static int emctask_startup()
{
	FILE *writeMsg = fopen("printMsg.txt", "a+");
	char msg[128];
	sprintf(msg, "the nml file is : %s \n", emc_nmlfile);
	fputs("enter the emctask_startup() to allocate resources!!!!!1\n", writeMsg);
	fputs(msg, writeMsg);
	fflush(writeMsg);
	fclose(writeMsg);
	emc_nmlfile;
...
int main(int argc, char *argv[])
{
	...
	FILE *writeMsg = fopen("printMsg.txt", "a+");
	fputs("Just a little testeeeeeeeeee@@@@@@@", writeMsg);
	fflush(writeMsg);
	//fclose(writeMsg);
	//delete writeMsg;
	int taskPlanError = 0;
	...
	fputs("***************get the command line-> for the fileName:emcGetArgs()****************\n", writeMsg);
	...
	// initialize globals
	set_rcs_print_destination(RCS_PRINT_TO_STDOUT);
	fputs("***************Set axis V to 1 emcInitGlobals()**************\n", writeMsg);
	emcInitGlobals();
	...
	//rcs_print("*********Load Hal File: emcRunHalFiles(emc_inifile);************\n");
	emcRunHalFiles(emc_inifile);

	// initialize everything
	set_rcs_print_destination(RCS_PRINT_TO_STDOUT);
	fputs("*******Alloccate source for everything emctask_startup():***********\n", writeMsg);
	...
	set_rcs_print_destination(RCS_PRINT_TO_STDOUT);
	fputs("*****emcTaskPlanInit(): inite the task planner******\n", writeMsg);
	...
	fputs("****************Start the task while cycle**************\n", writeMsg);
	while (!done) {
...
/****************************commiting*******************************/
			...
			//fflush(stdout);
			//fputs("**************Checking the hal layer pin:check_ini_hal_items()***************\n", writeMsg);
		
		check_ini_hal_items();
		// read command
		//rcs_print("****************Reading the Commandbuffer frome emcCommandbuffer******\n");
		//rcs_print("****************emcCommandBuffer->peek()******\n");
		...
		// run control cycle
		//fputs("***************Task planner working: emcTaskPlan***************\n", writeMsg);
		if (0 != emcTaskPlan()) {
			taskPlanError = 1;
		}
		//fputs("********************emcTask excutor working:emcTaskExecute()**********************\n", writeMsg);
		if (0 != emcTaskExecute()) {
			taskExecuteError = 1;
		}
		// update subordinate status
		//rcs_print("*******************Updating the io status:&emcStatus->io********************\n");
		emcIoUpdate(&emcStatus->io);
		//rcs_print("*************Updating the motion controller's status: &emcStatus->motion*******************\n");
		...
		if (emcStatus->io.aux.estop) {
			...
			//rcs_print("************Closing the lube**********\n");
			if (emcStatus->io.lube.on) {
				emcLubeOff();
			}
			//rcs_print("************Abort the spindle**********\n");
		...
		if (emcStatus->motion.status == RCS_ERROR ||
			...
			// clear out the pending command
			//rcs_print("**********Clear out the pending command buffer***********\n");
			emcTaskCommand = 0;
			interp_list.clear();
			emcStatus->task.currentLine = 0;
			...
			stepping = 0;
			steppingWait = 0;
			fputs("queue the command", writeMsg);
			// now queue up command to resynch interpreter
			emcTaskQueueCommand(&taskPlanSynchCmd);
		}
		//rcs_print("**********End of the checking for RCS_XXX***********\n");
		// update task-specific status
		//rcs_print("**********update task-specific status:emcTaskUpdate(&emcStatus->task);***********\n");
		emcTaskUpdate(&emcStatus->task);
		...
		//fputs("**********Write the emcStatus :emcStatusBuffer->write(emcStatus);************\n", writeMsg);
	...
	fputs("********end of while cycle*********\n", writeMsg);
	fclose(writeMsg);
	rcs_print("********end of while cycle*********\n");
	// end of while (! done)

	// clean up everything
	rcs_print("**********clean  up all of the resource********\n");
	emctask_shutdown();
	/* debugging */
	if (emcTaskNoDelay) {
		if (emc_debug & EMC_DEBUG_TASK_ISSUE) {
